...
- [ ] Translate P0 golden paths into failing tests before further feature changes; maintain case-to-test execution evidence.
...
- [ ] Write failing behavior tests: SQL rows -> Python summary -> JS output; cycle/mapping rejection; publish snapshots; duplicate admission and cancellation.
- [ ] Implement validation, publications, form schedule preview, authenticated CRUD/run/connection/runtime APIs.
- [ ] Implement per-language file protocol and compilation/cache, safe SQL parameter binding, driver readiness.
- [ ] Compile real n8n graph and authenticated execution adapter; persist node/run logs and artifacts; handle failure and stale leases.
- [ ] Verify real runtime roundtrip and targeted tests. Review against PRD before integration.
...
- [ ] Test graph editing/mapping state and bilingual schema before implementing graph model.
- [ ] Add workflow list/templates/editor with actual drag/drop/move/port connections, selection, undo and validation.
- [ ] Add input field picker, code configuration, connection/runtime management, publication/test/run inspection and no-Cron schedule forms.
- [ ] Integrate default-login card using bootstrap.local_account when supplied; keep v1 routes available.
- [ ] Verify syntax/model tests; integration owns live browser checks.
...
- [ ] Test local fresh bootstrap, existing-user preservation, changed-password hiding and service lifecycle policy.
- [ ] Implement local supervisor with single-instance lock, app/n8n/worker startup, AC/battery idle-sleep protection and stop/status commands.
- [ ] Implement double-click launcher with dependency readiness/errors; package app if available tooling allows; never claim signing without certificate.
- [ ] Offer explicit startup registration with user consent; do not silently modify host login items.
- [ ] Verify stopped/restart behavior and document exact implemented vs unavailable packaging capabilities.
...
- [ ] Mount APIs/local bootstrap and launch isolated local instance without touching existing state.
- [ ] Test immutable graph through actual n8n with SQL -> Python -> JS outputs and one scheduled occurrence.
- [ ] Inspect desktop English/Chinese UI, drag connections, saved workflows and run output via browser.
- [ ] Review security/lifecycle/duplicate and cancellation boundaries; fix findings and rerun covering tests.
- [ ] Run full regression suite, package checks and document evidence/remaining external release gates.
