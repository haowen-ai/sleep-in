...
## macOS C/C++ system consent

On macOS ARM64, candidate `apple-command-line-tools-arm64` supplies C and C++. It advertises `installer:"system_dialog"`, `system_consent:true`, `languages:["c","cpp"]`, the Apple license notice and official installation documentation. This candidate is an explicit exception to user-space archive installation: clicking Install requests `/usr/bin/xcode-select --install`, and macOS asks the user to approve the system installation and license. The application never runs `sudo`, accepts Apple's license, changes developer-directory selection or invokes installation automatically.

`GET /api/runtime-toolchains` only reads candidate/record metadata. The existing administrator-only `POST /api/runtime-toolchains/install` with `{manifest_id:"apple-command-line-tools-arm64"}` checks the selected real compiler paths first. If absent, it requests the system dialog and returns `status:"pending", phase:"system_consent", verification:{status:"not_run"}`. A successful dialog request is **not** toolchain readiness. Subsequent explicit Check installation clicks repeat the checks without reopening an outstanding dialog. `ready` is recorded only after both C11 and C++17 sources compile, link, execute and produce the exact expected output. A version string alone is insufficient. A failed dialog request or SDK/link failure returns `failed` with an actionable reason. Pending requests whose OS dialog was dismissed remain pending; finish installation using Apple's documented system flow before checking again.

A verified record supplies `executables:{c:".../clang",cpp:".../clang++"}` and `home`; runtime-pack selection resolves these exact language executables and still runs the normal JSON-file protocol pack self-test. The installation request tests mock the entire OS boundary; they do not prove an actual clean-machine installation or trigger an installation on the development Mac. [Apple's installation documentation](https://developer.apple.com/documentation/xcode/installing-the-command-line-tools) defines the system dialog and license step.

## Interrupted builds

Each runtime version and toolchain install owns a nonblocking advisory file lock. Dependency installers, compilers and protocol-test children inherit that lock, so an application crash does not permit a competing build while the original child is still active. Ready versions remain immutable. A read of a stale `building`/`installing` record with no remaining lock owner reconciles it to `failed` with `recovery_required:true`, allowing an explicit retry; a live owner leaves the active status unchanged and duplicate requests fail with an in-progress error. No GET starts a build or installation. An uncertain Apple system-dialog request remains `pending` instead of automatically requesting another system dialog.
