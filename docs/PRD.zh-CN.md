...
| 主画布 | 淡点阵背景、自由二维布局、四边独立输入/输出端口、方向箭头与避让；可选横向/纵向自动排版 |
...
节点位置不决定执行顺序。下游节点可以摆在上、下、左、右任意位置；四边分别提供实心输出端口与空心输入端口。箭头停在目标端口外，避免被圆点遮住；连线绕开无关节点。移动、平移和围绕鼠标缩放只改变显示。选中连线后在独立面板查看起点、终点和字段映射。横向/纵向自动布局是可撤销的可选操作。仅有依赖关系时可以不传数据，只有显式配置的输入映射才传值。
