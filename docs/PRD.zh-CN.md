...
Oracle 执行语法有效的 DDL 前会隐式提交，DDL 成功后也会提交。因此后续语句失败时，前面的更新和已经创建的表可能仍然保留。界面须显示运行失败并提醒核对外部影响，不得声称整个节点已回滚，也不能自动重放。[Oracle 事务说明](https://docs.oracle.com/en/database/oracle/oracle-database/26/sqlrf/COMMIT.html)。
...
所有方式都有时区、开始日期、可选结束日期。保存前显示自然语言摘要，例如“每周一至周五 07:30，上海时间；下次 9 月 21 日”，并列出带时区的未来五次执行。语言切换不改变时间语义。周期执行遇到 DST 缺失时刻跳过，重复时刻只取第一次；预览与执行必须调用相同计算规则。显式开始、结束和间隔锚点必须是存在的时刻：落在 DST 空洞时就地报错，不擅自移动边界；不带偏移量的输入按选定时区解析并持久化为 UTC。间隔计划必须保存固定起点，不能随预览或重启漂移。
...
默认同一流程不重叠：已有运行时，新触发记录为 skipped 并说明原因。高级策略可选顺序排队（有队列长度与到期上限），初版不开放同流程无限并行。默认对离线期间错过的计划记录跳过区间与原因，恢复后从下一次开始。可选仅补最近一次，且必须落在设定的宽限期内（默认两小时），不连续补跑积压任务。

## 11. 调试、保存、发布和运行记录

- **保存草稿**：允许未完成节点，保存配置与流程结构（节点位置由依赖自动生成）；提示最后保存时间。字段映射错误保留可见，不能以保存成功暗示可执行。
...
流程超时从编排开始计时，包含 n8n 导入与启动，不隐含额外启动宽限。确定性的输出校验错误、SQL 提交确认丢失不能自动重放业务。结果不确定后的新运行需明确提示，由用户决定。每个脚本的 stdout/stderr 文件各限 1 MiB，保留带截断提示的末尾日志；结构化输出按独立的数据/文件契约传递，不随日志截断。
