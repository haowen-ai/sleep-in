...
**状态：产品设计稿，尚未实现。** 本稿按用户提供的 11 张产品截图和最新要求重新设计，替代旧版“单个 Python 脚本定时执行”的产品方向。当前 localhost 与已发布代码仍为 v1；现有能力记录见 [验证记录](VERIFICATION.md)，历史需求见 [v1 PRD](archive/PRD-v1.md)。

## 1. 产品定位与设计原则

**把不同语言的脚本和数据库操作连成流程，让整条流程按时完成。**
...
| 参考 | 保留的产品能力 | 新设计 |
|---|---|---|
| 图 1：画布和大纲 | 开始节点、节点连接、大纲、保存、发布 | 横向流程画布；节点显示语言、运行环境与结果状态；保存与发布明确区分 |
...
画布支持添加、连线、复制、删除、撤销/重做、自动排版、缩放、适应画布、框选和搜索定位。复制生成新 ID；删除被引用节点时列出受影响映射，并使发布校验失败，不能静默丢弃数据引用。首版只允许有向无环图，禁止连出循环。
...
| 主画布 | 淡点阵背景、从左到右连线、自动排版；分支和汇合一眼可读 |
...
默认用户只有一台 MacBook，不需要服务器。主路径为 **安装 → 选模板 → 接数据 → 选时间 → 安心睡觉**。先提供一份可直接跑通的虚构周报模板，连接真实数据库是后续替换步骤。编辑代码、画流程图、配置环境属于进阶入口；日常用户只填普通表单。参数提供示例、默认值与就地校验，上游字段通过点选绑定，不要求写表达式。高级配置默认折叠。时间写成“每周一上午 7 点，America/Chicago”，不出现 Cron。
...
首页主按钮为 **Sleep in / 安心睡觉**，开启并保存 **自动托管定时任务** 偏好。每日、每周重复任务默认采用持续模式：配置一次，以后每天自动执行，不需要每天晚上再点。**今天早上跑完，不会关闭明天的保护。** 页面展示启用的流程、下一次时间、结果位置及当前就绪状态，默认不设“明早 09:00”截止。

一键检查电源、后台授权、应用/n8n/worker 健康、发布和环境状态、必需参数、当前数据库/网络/VPN 连通性、下一次计划与输出空间。失败给出具体修复入口。就绪带检查时间，不保证外部连接未来永远正常。预检不执行写数据库等副作用操作；首次引导试跑使用虚构数据。

助手在启动/登录、计划修改、电源变化时及定期重新核对保护状态：**开启托管 + 已插电 + 至少一个已启用的未来定时触发器或正在执行的定时任务**，就保持防空闲休眠请求。**等待下一次运行的间隔也必须保护**，包括白天、两次每周任务之间的日期；不假设电脑睡着后会自行到点醒来。已暂停、删除或到期的触发器不计入。暂时因连接/环境出错但仍启用的计划继续计入并显示“需要处理”，避免一次故障悄悄取消后续保护。

允许正常锁屏、熄屏，不永久改系统全局电源设置。状态显示“自动托管中 · 下次周一 7:00 · 请插电并保持开盖”，附最近检查时间；详情说明插电期间会在计划间隔持续保持系统唤醒。关闭浏览器不停止助手。上一次执行失败不自动取消明天的计划，重试和并发按各流程策略处理。

所有定时触发器均暂停/删除/到期且活跃定时运行结束后释放保护；关闭托管、退出应用或拔电则立即释放，任务本身仍遵守超时。释放保护不删除原有定时。用户主动停止托管后记住这个选择，不能下次插电又偷偷开启。若托管偏好仍开启，重新插电或重启后登录就自动检查服务、重新申请保护并恢复调度。进程崩溃释放请求；后台管理器重启助手后依据持久偏好恢复。明确显示未保护的空档和错过的任务，不能把崩溃期间宣称为持续受保护。

可选的 **保护到指定时间** 只用于临时场景，明确展示到期时间；到期最多额外等待已开始任务 60 分钟，然后释放。不能删除之后的重复计划，但必须提示这些未来执行时间不再受保护，并提供“改为自动托管”。每日/每周模板不默认选择临时模式。

| Mac 状态 | 产品行为与明确边界 |
|---|---|
| 锁屏/熄屏但系统醒着 | 后台可继续运行，不用开着网页 |
| 插电且自动托管开启 | 覆盖重复计划的完整等待时间，持续请求防空闲休眠，而非只保护一晚 |
| 合盖或主动点“睡眠” | 不保证执行，不承诺越过这些系统状态 |
| 拔电 | 释放保护、保留托管偏好，显示保护暂停；电脑醒着时可能执行，但不保证 |
| 再次插电 | 托管偏好仍开启时自动检查并恢复保护，不需要再次点击 |
| 关机或退出系统账号 | 无法保证执行；用户再次登录且服务健康后恢复 |
| 网络/VPN 中断 | 本地任务可能继续；远程任务按故障策略处理，后续定时所需的电源保护继续保留 |

唤醒/重启后列出错过的计划和原因。默认跳过并留记录；可选“只补跑最近一次”，宽限默认两小时且可配置。不自动补跑全部积压，也不重复结果未知的写入；**下一次未来计划保持启用**。系统通知可选，启用后再请求通知权限。菜单栏提供状态、结果、暂停/恢复托管和退出。首版不安排系统自动唤醒，不保障接外屏合盖的特殊配置。
...
| A19 | 插电、开盖、开启保护后，锁屏/熄屏状态真实完成定时流程；单独核验空闲休眠请求；关网页不停止后台 |
| A20 | 连续两天按时执行，首日完成后不解除后续保护且无需再点；全暂停释放、重新插电/登录恢复、主动停止后不擅自开启；补跑不重复写入 |
| A21 | 后台授权同意/拒绝/撤销、登录后恢复、签名安装、升级回滚、时区变化在每个宣传支持的 Mac 平台实测 |
| A22 | 新手最多经三个安装后产品页面完成虚构模板；默认值、错误、结果位置及中英文均无需写代码或 Cron |
| A23 | 临时到期、一次性完成、助手崩溃及撤权正确释放请求；重复任务失败仍保留未来计划；跨日/跨周等待及 DST 实测 |
...
下一步应先完成阶段 A 的执行适配验证与编辑器视觉稿，再推进代码改造；本 PRD 不把尚未落地的工作流能力写成已交付功能。

Apple 官方说明：防空闲系统休眠请求允许熄屏，但不能阻止合盖、主动睡眠或低电量导致的睡眠。因此首版要求插电、开盖，不以一次授权承诺全天候执行。[Apple 电源请求](https://developer.apple.com/documentation/iokit/kiopmassertiontypepreventuseridlesystemsleep)。后台服务注册可能需要用户同意，助手应读取授权状态并引导用户操作。[Apple 后台注册](https://developer.apple.com/documentation/servicemanagement/smappservice/register%28%29)。签名安装、原生运行环境兼容性、本地存储并发和电源状态均须在发布前验证，当前仍是设计要求。
