...
面向两类用户：流程作者负责脚本、连接和节点配置；日常使用者从模板或已发布流程出发，只需要填写参数、选择时间、查看结果。普通用户无需学习 n8n 编辑器、Cron、Docker 命令或内部节点协议；部署者仍需启动并持续运行服务。
...
| RuntimeProfileVersion | 语言、工具链、镜像摘要、锁定依赖、共享模块、编译/启动规则与限制 |
...
| Shell | Bash / sh 及版本、可用命令、共享 shell 文件 | 明确 Linux 环境；输入经 JSON 文件或分离 argv 传递，不能拼成命令字符串 |
...
| Custom / 自定义 | 注册语言标识、固定镜像摘要、构建/运行 argv、文件协议 | 管理员注册并通过协议自测后可用于节点；未安装时展示就绪状态与修复入口 |
...
执行器按固定环境运行，在受限目录内读写；设置 CPU、内存、执行时间和磁盘限制。Web 与 n8n 不直接获得 Docker socket。环境构建与执行的宿主权限由单独的受控运行服务管理；首个交付采用预配置的语言 worker 服务，自定义镜像须管理员安装。不是陌生人代码托管服务，不把普通容器宣称为完整安全沙箱。
...
保留 clone → Docker Compose → 一次性初始化 → 示例流程的路径。基础安装包含 n8n、应用、存储、Python/Node.js/Shell/SQL worker。演示流程不连接外部数据库、不发通知。

Java 和 C/C++ 属于第一阶段的验收目标，以可选运行服务安装；默认不把所有大型工具链都拉到每位用户机器上。界面区分“可安装 / 安装中 / 就绪 / 不可用”，附准确的安装命令；启用后才可发布相关节点。自定义镜像的安装步骤由管理员完成，网页不能悄悄修改宿主 Docker。

| 阶段 | 交付与验收闸门 |
|---|---|
| A：纵向样例 | SQLite → Python → JavaScript，图形映射 + 真实 n8n 顺序编排 + 文件输出 + 表单定时 + 中英文；先证明贯通 |
| B：完整首版 | Python/JS/Shell，PostgreSQL/MySQL/SQLite/Oracle 接收器；Java/C/C++ 可选工具链、统一环境；发布、输入校验、分支/合并、日志、通知设置、取消与恢复 |
| C：扩展 | 更多数据库/语言、可视化逐项处理、循环、嵌套流程、断点续跑、更多通知预设、性能与规模优化 |

阶段 A 不得对外宣称完整多语言支持。Oracle、Java、C/C++ 必须完成真实运行验证才可以进入阶段 B 发布说明，不能用图标和下拉选项充数。

首版不做 DeepModel 查询、业务对账节点、财务模型、维度模型、自动生成业务模板、SaaS 计费或无限量插件商店。业务逻辑统一交给 SQL 与脚本。外部 HTTP 调用由脚本完成；最终文件来自脚本输出契约，不另造一组业务功能节点。
