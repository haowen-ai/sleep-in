...
def test_browser_type_invalid_fallback_and_runtime_null_never_execute_consumer(engine_url, tmp_path):
 ...
 const uncertain=await seed({nodes:[node('source',{source:'def main(inputs): return {"rows":None}',outputs:{type:'object',properties:{rows:{}}}}),node('consumer',{source:counted})],edges:[]});await open(uncertain);await mapping('consumer','orders','node',null,{source:'source',path:'rows',type:'array',optional:true,fallback:[]});saved=await save(uncertain);assert.equal(saved.validation_errors.length,0);const run=await testRun(uncertain);assert.equal(run.status,'failed');assert.equal(run.nodes.source.status,'succeeded');assert.equal(run.nodes.source.output.data.rows,null);assert.equal(run.nodes.consumer.status,'failed');assert.equal(run.nodes.consumer.attempts.length,0);assert.match(run.nodes.consumer.error,/orders.*array|array.*orders/);assert.deepEqual(run.nodes.consumer.inputs,{});await page.route('**/api/workflow-runs/'+run.id,async route=>{await new Promise(resolve=>setTimeout(resolve,250));await route.continue();});await page.goto(origin+'/workflow-runs/'+run.id);await page.waitForFunction(()=>document.body.textContent.includes('array'));
 """)
    assert count(counter)==[]
    assert len(durable_runs(tmp_path))==1
