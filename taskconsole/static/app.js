...
function healthContents(){const health=schedulerHealth(state.bootstrap||{},route());return [el('span',{title:health.reasonKey?t(health.reasonKey):health.reason||''},badge(health.status||'unavailable')),el('span',{'class':'health-caption'},isWorkflowRoute(route())?t('admin.workflowScheduler'):t('admin.scheduler'))];}
...
async function loadEngineHealth(){try{return await api('/api/workflow-health');}catch{return {status:'unavailable'};}}
setInterval(async()=>{if(!state.bootstrap?.user||!isWorkflowRoute(route()))return;try{const [current,engine]=await Promise.all([api('/api/bootstrap'),loadEngineHealth()]);state.bootstrap.workflow_scheduler=current.workflow_scheduler;state.bootstrap.workflow_engine=engine;refreshHealth();}catch{state.bootstrap.workflow_scheduler={status:'unavailable'};refreshHealth();}},10000);
async function bootstrap(){
  state.locale=normalizeLocale(localStorage.getItem('taskconsole.locale'));
  try{state.bootstrap=await api('/api/bootstrap');setCsrf(state.bootstrap.csrf); if(state.bootstrap.user){state.locale=normalizeLocale(state.bootstrap.user.locale||state.locale);state.bootstrap.workflow_engine=await loadEngineHealth();} render();}
